...
func update_brake_light(brake: float, engine_on: float) -> void:
	...
	$RearLightOn/RearLightBraked.modulate.a = smoothstep(0.0,0.2,brake)
...
#		engine_torque += 50*dt*clampf(50*vel_diff,-max_torque_bound,max_torque_bound)
		var torque_curve = 0.7*max(20 + engine_ang_vel*0.164 - engine_ang_vel*engine_ang_vel*0.0002,20)
...
func _physics_process(delta):
	...
	GlobalVars.engine_rpm = abs(to_rpm(engine_ang_vel))
